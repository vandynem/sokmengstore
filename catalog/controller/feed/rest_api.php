...
class ControllerFeedRestApi extends RestController
{
    ...
    */
	...
	{
		$settings = parse_ini_file('/var/www/html/settings.ini');
			$this->json["data"] =$settings;
	}
...
//            );
    ...
    public function edittoken($post)
    {
        
      
        
        if (!isset($post['toekn']) ) {
            $this->json['error'] = "Token Reuired";
        }
     ...
     //   if (empty($this->json["error"])) {
            $this->load->model('account/customer');
            $this->model_account_customer->editToken($post);
          //  $data['msg']="ok";
...
//Options By Avinash 31/01/2020

$data_options=$this->model_catalog_product->getProductOptions($product['product_id']);
            ...
            foreach ($slideshows as $slideshow) {
                ...
                if(!empty($results)) {
                    foreach ($results as $result) {
                        ...
                        if (strpos($result['title'], '_') !== false) {
 $str_arr = explode ("_", $result['title']);
 $image2= $this->model_tool_image->resize($result['image'], $str_arr[1], $str_arr[2]);   
}else if($result['title']=='Advertisement')
{
$image2= $this->model_tool_image->resize($result['image'], 1080, 2280);
}
else
{
$image2=$this->model_tool_image->resize($result['image'], $module_info['width'], $module_info['height']);
}
