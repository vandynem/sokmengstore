...
class ControllerApiOrder extends Controller {
	...
	public function history() {
		...
		if (!isset($this->session->data['api_id'])) {
			...
			$order_info = $this->model_checkout_order->getOrder($order_id);
			$log = new Log('notification.log');
		//	 $log->write("order status id");
			 
		//	  $log->write($this->request->post['order_status_id']);
			 // $order_status="";
			  
			 // Added by Avinash firebase
			 $this->load->model('account/customer');
			 $customer_info = $this->model_account_customer->getCustomer($order_info['customer_id']);
			 // $log->write("customer inf");
			// $log->write($customer_info);
			 
			  $orderstatusid=$this->request->post['order_status_id'];
			   $log->write("User order status");
			 $log->write($orderstatusid);
			  $order_status ="";
			...
			if ($orderstatusid=='3')
			   ...
			   if ($orderstatusid=='16')
			 {
				 $order_status ="Voided";
						...
						$message = 'Dear '.$customer_info['firstname']. ' Your Order No. '.$order_info['order_id'].' has been updated. Your order is now '.$order_status ;
