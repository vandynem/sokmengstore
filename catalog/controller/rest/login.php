...
class ControllerRestLogin extends RestController
{
    ...
    const FACEBOOK_USER_INFORMATION_URL = 'https://graph.facebook.com/me?fields=email,name,id';
                           ...
                           //  $post['email']=$ret['openid'].'@noemailkonfulononline.com';
							...
							$post['email']= $ret['openid'].'@noemailkonfulononline.com';
                             ...
                             $customer_info = $this->model_account_customer->getCustomerByCode($post['code']);
                              $this->log->write('After run model');  
                             if (empty($this->json['error'])) {
                                 ...
                                 if ($customer_info) {
										 ...
										 $this->log->write('customer info status :'.$customer_info['status']);  
										 
										 $customer_info['status']=1;
										 $this->log->write('After assing customer info status :'.$customer_info['status']);  
    ...
    public function sociallogin()
    {
        ...
        if ($_SERVER['REQUEST_METHOD'] === 'POST') {
            ...
            if (empty($this->json['error'])) {
                $this->load->model('account/customer');
 $this->log->write("before customer  model :");
                $customer_info = $this->model_account_customer->getCustomerByEmail($post['email']);
 $this->log->write("after customer  model :");
  $this->log->write($customer_info);
...
if ($social['email'])
        ...
        $this->log->write("New Soccial emai Email :".$social['email']);
                ...
                if (empty($this->json['error'])) {
                    //if email does not exist, register as a new customer
                    if (!$customer_info) {
                        $this->log->write("befor cust info");
                        $data['email'] = $post['email'];
                        $data['firstname'] = $firstname;
                        ...
                        $this->log->write("befor cust add ");
                        $customer_info = $this->model_account_customer->getCustomerByEmail($post['email']);
                        $this->log->write("after get cust");
                    }

                    if ($customer_info) {
                        if (!$this->customer->login($post['email'], "", true)) {
							$this->log->write("in post email err");
                            $this->json['error'][] = $this->language->get('error_login');
                            $this->statusCode = 400;
                        }
                    }
            ...
            if (empty($this->json['error']) && !empty($customer_info)) {
                unset($this->session->data['guest']);
$this->log->write("not err not info blank");
                // Default Addresses
                $this->load->model('account/address');
$this->log->write("after load addre");
                if ($this->config->get('config_tax_customer') == 'payment') {
                    $this->session->data['payment_address'] = $this->model_account_address->getAddress($this->customer->getAddressId());
                }
...
$this->log->write("after get data");
                ...
                // Custom Fields
                $this->load->model('account/custom_field');
$this->log->write("after get customer field");
                $customer_info['custom_fields'] = $this->model_account_custom_field->getCustomFields($this->config->get('config_customer_group_id'));
...
$this->log->write("end facebook logi");
        $this->sendResponse();
    }
