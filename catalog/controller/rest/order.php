...
class ControllerRestOrder extends RestController
{
  ...
  $this->load->model('tool/image');
            ...
            foreach ($results as $result) {
                ...
                $productresults = $this->model_account_order->getOrderProductsDetails($result['order_id']);
                 $this->log->write($result['order_id']);
                foreach ($productresults as $productresult) {
					 //$this->log->write($productresult['product_id']);
             
			 $images = array();
			$original_images = array();
			  $additional_images = $this->model_catalog_product->getProductImages($productresult['product_id']);
            ...
            if(!empty($additional_images)) {
                foreach ($additional_images as $additional_image) {
                    ...
                    $products[] = array(
                        ...
                        'product_model'  => $productresult['model'],
                        'quantity'  => $productresult['quantity'],
                        'price'  => $productresult['price'],
                        'total'  => $productresult['total'],
                        'tax'  => $productresult['tax'],
                        'image'  => "https://konfulononline.com/image/".preg_replace('~\s+~', '%20',$productresult['image']),
						'images' => $images,
