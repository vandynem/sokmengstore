...
class ControllerRestConfirm extends RestController
{
    ...
    public function confirm()
    {
        ...
        $this->checkPlugin();
  $this->log->write("Confrim API");
    $this->log->write($_SERVER['REQUEST_METHOD']);
   ...
   $this->log->write("saveOrderToDatabase");
    $this->log->write("pay method");
	 $this->log->write($this->session->data['payment_method']);
        $this->load->model('checkout/order');
        $this->load->model('account/order');
  $this->log->write("saveOrderToDatabase after load model");
...
$this->log->write($cod_status);
            ...
            if (!isset($this->session->data['payment_method']) || empty($this->session->data['payment_method'])) {
				...
				$this->log->write("saverorder if set payment method");
                ...
                if (empty($status)) {
				   ...
				   $defaultStatus = $this->config->get("payment_" . $this->session->data['payment_method'] . '_order_status_id');
                    $defaultStatus = is_null($defaultStatus) ? $order_status_id : $defaultStatus;
$this->log->write("saverorder before checkout order");
$this->log->write( $defaultStatus);
$this->log->write( $this->session->data['comment']);
				  ...
				  $this->model_checkout_order->addOrderHistory($this->session->data['order_id'], $defaultStatus, "Sucessucess", false);
					$this->log->write("saverorder after checkout order");
                }
            }
...
$this->log->write("set payment code ".$order_data['payment_code']);
   ...
   $this->log->write("Payment Method Session Data");
      $this->log->write($this->session->data['payment_method']);
      $this->log->write($this->session->data['payment_method']);
       ...
       // if (isset(static::$paymentMethods[$this->session->data['payment_method']['code']])) {
		    if (isset(static::$paymentMethods[$this->session->data['payment_method']])) {
         //   $method = static::$paymentMethods[$this->session->data['payment_method']['code']];
		 $method = static::$paymentMethods[$this->session->data['payment_method']];
	  ...
	  $this->log->write("Pay class response");
	  	  $this->log->write($data);
