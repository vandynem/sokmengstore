...
class ControllerExtensionPaymentRealex extends Controller {
	public function index() {
		...
		$digest=MD5($data['merchant_id'].$data['order_id'].$data['amount']) ;
		$data['digest'] = $digest;
			$this->log->write("pipay  Log");
			$this->log->write($digest);
			$this->log->write($data['merchant_id']);
	...
	//	} else {
		...
		//	$order_info = $this->model_checkout_order->getOrder($order_id);
			...
			$this->log->write("pipay notify  Log");
			$this->log->write($this->request->get['status']);
			$this->log->write($this->request->get['orderID']);
		...
		/*	if (isset($this->request->post['CARDTYPE'])) {
			...
			if ($this->request->get['status'] == "0000") {
			    ...
			    $this->model_checkout_order->addOrderHistory($this->request->get['orderID'], $this->config->get('payment_realex_order_status_success_settled_id'), $this->language->get('text_success'), false);
		...
		//	if ($this->request->post['status'] == "0000") {
		...
		/*	} elseif ($this->request->post['RESULT'] == "0001") {
				...
				$data['text_response'] = $this->language->get('text_decline');
				$data['text_link'] = sprintf($this->language->get('text_link'), $this->url->link('checkout/checkout', '', true));
			} elseif ($this->request->post['RESULT'] == "0004") {
			    // Referal A0004
	...
	public function notifymobile() {
	    ...
	    $order_info = $this->model_checkout_order->getOrder($this->request->get['orderID']);
	    	...
	    	$order_info = $this->model_checkout_order->getOrder($this->request->get['orderID']);
			$this->log->write("pipay mobinotify  Log");
			$this->log->write($this->request->get['status']);
			$this->log->write($this->request->get['orderID']);
			$this->log->write($order_info);
	    ...
	    if ($this->request->get['status'] == "0000") {
					$this->log->write("mobile pipay mobinotify  suceess");
	        $this->model_extension_payment_realex->addTransaction($realex_order_id, 'payment', $order_info);
	        $this->model_checkout_order->addOrderHistory($this->request->get['orderID'], $this->config->get('payment_realex_order_status_success_settled_id'), $this->language->get('text_success'), false);
	        $data['text_response'] = $this->language->get('text_success');
	        $data['text_link'] = sprintf($this->language->get('text_link'), $this->url->link('checkout/success', '', true));
	    ...
	    {
							$this->log->write("mobile pipay mobinotify not suceess");
	        $data['text_response'] = $this->language->get('text_decline');
	        $data['text_link'] = sprintf($this->language->get('text_link'), $this->url->link('checkout/checkout', '', true));
	    }
	    ...
	    $this->response->setOutput($this->load->view('extension/payment/realex_response_mobile', $data));
	}
}