...
class ControllerExtensionPaymentGlobalpay extends Controller {
	...
	//	}
		...
		$this->log->write("total : ".$order_info['total']);
	...
	public function notify() {
		...
		$this->load->language('extension/payment/globalpay');
			...
			$data['api_key'] ='ba089bcd3bafd8323c2013d3e0342672';
			
			$data['merchant_id'] = 'konfulon';
			$data['transactionId'] = $this->session->data['order_id'];
			...
			$data['hash'] = base64_encode(hash_hmac('sha512', $data['merchant_id'] .$order_id, $data['api_key'], true));
			...
			$this->log->write("merchant_id : ".$data['merchant_id']);
			$this->log->write("api : ".$data['api_key']);
			$this->log->write("Total : ".$data['amount']);
			$this->log->write("Total for checkout: ".$data['order_totalforcheckout']);
			
			$this->log->write("hash : ".$data['hash']);
		...
		//	$this->log->write(print_r($order_info));
			...
			//	if ($auto_settle == 1) {
					...
					$this->model_checkout_order->addOrderHistory($order_id, '2', $this->language->get('text_success'), false);
				//} else {
	...
	//	} else {
	  ...
	  // $data['hash'] = base64_encode(hash_hmac('sha512', $data['merchant_id'].$data['transactionId'],$data['api_key'] , true));
			...
			$this->log->write("globalpay order status");
			$this->log->write($data['transactionId']);
	    ...
	    if ($order_status['status'] == "0") {
	        ...
	        if ($auto_settle == 1) {
	            $this->model_extension_payment_globalpay->addTransaction($globalpay_order_id, 'payment', $order_info);
	            $this->model_checkout_order->addOrderHistory($order_id, $this->config->get('globalpay_order_status_success_settled_id'), $this->language->get('text_success'), false);
	        } else {
	            $this->model_extension_payment_globalpay->addTransaction($globalpay_order_id, 'auth', 0.00);
	            $this->model_checkout_order->addOrderHistory($order_id, $this->config->get('payment_globalpay_order_status_success_unsettled_id'), $this->language->get('text_success'), false);
	        }
