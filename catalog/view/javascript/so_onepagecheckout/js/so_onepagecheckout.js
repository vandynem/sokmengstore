...
//var default_city_id = $('#default_city_id').val();
	
	$('.so-onepagecheckout form.form-shipping select[name="shipping_zone_id"]').on('change', function(e, first){
		if(!this.value) return;
		$.ajax({
			...
			cache:false,
			beforeSend:function(){
				$('.so-onepagecheckout form.form-shipping select[name="shipping_zone_id"]').after(' <i class="fa fa-circle-o-notch fa-spin"></i>');
			},
			complete:function(){
				$('.so-onepagecheckout .fa-spin').remove();
			},
			success:function(json){
				...
				if(json['city']!=''){
					for(i=0;i<json['city'].length;i++){
						...
						if(json['city'][i]['city_id']==default_city_id){
							html+=' selected="selected"';
						}
				...
				$('.so-onepagecheckout form.form-shipping select[name="shipping_city_id"]').html(html);
	...
	$('.so-onepagecheckout form.form-shipping select[name="shipping_cityid"]').on('change', function(){
		$(document).trigger('so_checkout_address_changed', 'shipping');
	});
	...
	var default_city_id = $('.so-onepagecheckout #default_city_id').val();
	$('.so-onepagecheckout form.form-payment select[name="payment_zone_id"]').on('change',function(e,first){
		if(!this.value)return;
		$.ajax({
			url:'index.php?route=checkout/checkout/zone&zone_id='+this.value,
			dataType:'json',
			cache:false,
			beforeSend:function(){
				$('.so-onepagecheckout form.form-payment select[name="payment_zone_id"]').after(' <i class="fa fa-circle-o-notch fa-spin"></i>');
			},
			complete:function(){
				$('.so-onepagecheckout .fa-spin').remove();
			},
			success:function(json){
				...
				$('.so-onepagecheckout form.form-payment select[name="payment_city_id"]').html(html);
	...
	$('.so-onepagecheckout form.form-payment select[name="payment_city_id"]').on('change',function(){
		$(document).trigger('so_checkout_address_changed', 'payment');
	});
			...
			success:function(json){
				...
				if(json['errors']){
					$.each(json['errors'],function(k,v){
						...
						if($.inArray(k,['payment_country','payment_zone','payment_city','shipping_country','shipping_zone','shipping_city'])!==-1){
							k+='_id';
	...
	$(document).on('so_checkout_address_changed', function(e, type){
		...
		if($('.so-onepagecheckout input[name="'+type+'_address"]:checked').val()==='existing'){
			...
			data[type+'_city_id']=$('select[name="'+type+'_city_id"]').val();
			if(type==='payment'&&$('input[name="shipping_address"]').is(":checked")){
				...
				data['shipping_city_id']=$('select[name="'+type+'_city_id"]').val();
			}
		}
