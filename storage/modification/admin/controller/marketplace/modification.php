...
$this->load->model('extension/module/modification_manager');
		...
		$modification_total = $this->model_extension_module_modification_manager->getTotalModifications();
...
$this->load->model('extension/module/modification_manager');
		...
		foreach ($results as $result) {
			$data['modifications'][] = array(
 'date_modified' => $result['date_modified'] && $result['date_modified'] != '0000-00-00 00:00:00' ? date(date('Ymd') == date('Ymd', strtotime($result['date_modified'])) ? 'G:i' : $this->language->get('date_format_short'), strtotime($result['date_modified'])) : null,
